...
func (s *Store) ListGroups(ctx context.Context) ([]store.Group, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, uri, display_name, description, enabled, created_at, updated_at FROM groups ORDER BY display_name, uri`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&v.ID, &v.URI, &v.DisplayName, &v.Description, &enabled, &created, &updated); err != nil {
			return nil, err
		}
		v.Enabled = scanBool(enabled)
		v.CreatedAt = parseTime(created)
...
func (s *Store) GetGroup(ctx context.Context, id string) (*store.Group, error) {
	...
	var enabled int
	var created, updated string
	err := s.db.QueryRowContext(ctx, s.q(`SELECT id, uri, display_name, description, enabled, created_at, updated_at FROM groups WHERE id = ?`), id).
		Scan(&v.ID, &v.URI, &v.DisplayName, &v.Description, &enabled, &created, &updated)
	if ok, err := notFound(err); ok || err != nil {
		return nil, err
	}
	v.Enabled = scanBool(enabled)
