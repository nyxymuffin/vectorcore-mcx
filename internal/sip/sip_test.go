...
func TestParseMultipartMCPTTInfo(t *testing.T) {
	raw := "PUBLISH sip:mcptt-as@ims.mnc435.mcc311.3gppnetwork.org SIP/2.0\r\n" +
		"From: <sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org>;tag=1\r\n" +
		"To: <sip:mcptt-as@ims.mnc435.mcc311.3gppnetwork.org>\r\n" +
		"Call-ID: pub-1\r\n" +
		"CSeq: 1 PUBLISH\r\n" +
		"Event: poc-settings\r\n" +
		...
		"<mcpttinfo xmlns=\"urn:3gpp:ns:mcpttInfo:1.0\"><mcptt-Params><mcptt-client-id type=\"Normal\"><mcpttString>sip:16752012881@ims.mnc435.mcc311.3gppnetwork.org</mcpttString></mcptt-client-id></mcptt-Params></mcpttinfo>\r\n" +
		"--abc--\r\n"
	...
	if got := mcpttIdentityFromBody(msg); got != "sip:16752012881@ims.mnc435.mcc311.3gppnetwork.org" {
		t.Fatalf("mcptt identity = %q", got)
	}
}

func TestMCPTTIdentityPrefersRequestURIOverAccessToken(t *testing.T) {
	raw := "SUBSCRIBE sip:mcptt-as@ims.mnc435.mcc311.3gppnetwork.org SIP/2.0\r\n" +
		"From: <sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org>;tag=1\r\n" +
		"To: <sip:mcptt-as@ims.mnc435.mcc311.3gppnetwork.org>\r\n" +
		"Call-ID: sub-1\r\n" +
		"CSeq: 1 SUBSCRIBE\r\n" +
		...
		"<mcptt-client-id type=\"Normal\"><mcpttString>sip:16752012881@ims.mnc435.mcc311.3gppnetwork.org</mcpttString></mcptt-client-id>" +
		"<mcptt-request-uri type=\"Normal\"><mcpttURI>sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org</mcpttURI></mcptt-request-uri>" +
	...
	if got := mcpttIdentityFromBody(msg); got != "sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org" {
		t.Fatalf("mcptt identity = %q", got)
	}
}

func TestReadSIPMessageFromTCPStream(t *testing.T) {
	stream := "OPTIONS sip:mcptt-as@ims.mnc435.mcc311.3gppnetwork.org SIP/2.0\r\n" +
		"Call-ID: one\r\nContent-Length: 0\r\n\r\n" +
		"OPTIONS sip:mcptt-as@ims.mnc435.mcc311.3gppnetwork.org SIP/2.0\r\n" +
		"Call-ID: two\r\nContent-Length: 4\r\n\r\ntest"
...
func TestPresenceBodyMatchesMCOPShape(t *testing.T) {
	s := &Server{}
	mcpttID := "sip:16752012881@ims.mnc435.mcc311.3gppnetwork.org"
	body := s.presenceBody(mcpttID, "sip:DEMO_group@ims.mnc435.mcc311.3gppnetwork.org", testTime())

	for _, want := range []string{
		`<presence xmlns="urn:ietf:params:xml:ns:pidf"`,
		`xmlns:mcpttPI10="urn:3gpp:ns:mcpttPresInfo:1.0"`,
		`entity="sip:16752012881@ims.mnc435.mcc311.3gppnetwork.org"`,
		`<tuple id="sip:16752012881@ims.mnc435.mcc311.3gppnetwork.org">`,
		`<contact priority="1.0">sip:16752012881@ims.mnc435.mcc311.3gppnetwork.org</contact>`,
		`<mcpttPI10:affiliation group="sip:DEMO_group@ims.mnc435.mcc311.3gppnetwork.org" status="affiliated"`,
	} {
...
func TestXCAPDiffBodyListsMCOPStartupDocuments(t *testing.T) {
	...
	ctx := context.Background()
	user, err := st.CreateUser(ctx, store.User{IMPI: "u@example.test", IMPU: "sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org", MCPTTID: "sip:mcptt-u@example.test", DisplayName: "User", Enabled: true})
	if err != nil {
		t.Fatal(err)
	}
	...
	s := NewServer(cfg, st)
	body := s.xcapDiffBody(ctx, "sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org", nil)

	for _, want := range []string{
		`<xcap-diff xmlns="urn:ietf:params:xml:ns:xcap-diff"`,
		`xcap-root="http://10.90.250.54:8100/xcap-root"`,
		`sel="org.3gpp.mcptt.ue-config/users/mcptt_UE_id/mcptt_UE_id"`,
		`sel="org.3gpp.mcptt.user-profile/users/sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org/mcptt-user-profile"`,
		`sel="org.3gpp.mcptt.service-config/global/service-config.xml"`,
		`sel="org.openmobilealliance.groups/global/byGroup/sip:DEMO_group@ims.mnc435.mcc311.3gppnetwork.org"`,
	} {
		if !strings.Contains(body, want) {
	...
	for _, bad := range []string{
		...
		`sel="org.3gpp.mcptt.user-profile/users/sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org/"`,
	} {
		if strings.Contains(body, bad) {
			t.Fatalf("xcap-diff included directory selector %q:\n%s", bad, body)
		}
	}
	if body != s.xcapDiffBody(ctx, "sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org", nil) {
...
func TestXcapDiffBodyUsesRequestedSelectors(t *testing.T) {
	...
	cfg.CMS.XCAPRoot = "http://10.90.250.54:8100/xcap-root"
	s := NewServer(cfg, nil)
	body := s.xcapDiffBody(context.Background(), "sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org", []string{
		"org.3gpp.mcptt.ue-config/users/mcptt_UE_id/",
		"/org.3gpp.mcptt.user-profile/users/sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org/",
...
func TestXcapResourceListSelectorsFromMultipartSubscribe(t *testing.T) {
	raw := "SUBSCRIBE sip:mcptt-as@ims.mnc435.mcc311.3gppnetwork.org SIP/2.0\r\n" +
		...
		"<entry uri=\"org.3gpp.mcptt.user-profile/users/sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org/\"/>" +
	...
	want := []string{
		...
		"org.3gpp.mcptt.user-profile/users/sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org/mcptt-user-profile",
		"org.3gpp.mcptt.service-config/global/service-config.xml",
	}
	if strings.Join(got, "\n") != strings.Join(want, "\n") {
...
func TestNotifyRequestURIUsesRemoteTargetWhenRouted(t *testing.T) {
	sub := store.Subscription{
		SubscriberURI: "sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org",
		RemoteTarget:  "sip:311435300070581@192.168.105.116:45911;transport=udp",
	}

	got := notifyRequestURI(sub, []string{"<sip:orig@scscf.ims.mnc435.mcc311.3gppnetwork.org:5060;lr>"}, "ims.mnc435.mcc311.3gppnetwork.org")
...
func TestNotifyRequestURIFallsBackToRemoteTargetWithoutRoutes(t *testing.T) {
	...
	got := notifyRequestURI(sub, nil, "ims.mnc435.mcc311.3gppnetwork.org")
...
func TestResponsePreservesRepeatedVia(t *testing.T) {
	msg, err := Parse([]byte("OPTIONS sip:mcptt-as@example.test SIP/2.0\r\n" +
		"Via: SIP/2.0/TCP 10.90.250.50;branch=z9hG4bK1\r\n" +
...
func TestPublishResponseUsesUnquotedSIPETag(t *testing.T) {
	...
	s := NewServer(config.Default(), st)
	raw := "PUBLISH sip:mcptt-as@ims.mnc435.mcc311.3gppnetwork.org SIP/2.0\r\n" +
		"Via: SIP/2.0/UDP 192.168.105.116:40506;branch=z9hG4bKpub\r\n" +
		"From: <sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org>;tag=pub\r\n" +
		"To: <sip:mcptt-as@ims.mnc435.mcc311.3gppnetwork.org>\r\n" +
		"Call-ID: pub-etag\r\n" +
		"CSeq: 1 PUBLISH\r\n" +
		"Event: poc-settings\r\n" +
		...
		"Content-Length: 0\r\n\r\n"
	var resp []byte
	s.handleRaw(t.Context(), "192.168.105.116:40506", "udp", []byte(raw), func(b []byte) error {
...
func TestThirdPartyRegisterCreatesRegistration(t *testing.T) {
	...
	cfg.SIP.AdvertiseHost = "10.90.250.54"
	cfg.SIP.AdvertisePort = 5060
	s := NewServer(cfg, st)
	raw := "REGISTER sip:mcptt-as.ims.mnc435.mcc311.3gppnetwork.org:5060 SIP/2.0\r\n" +
		"Via: SIP/2.0/UDP 10.90.250.52:5060;branch=z9hG4bKreg\r\n" +
		"From: <sip:scscf.ims.mnc435.mcc311.3gppnetwork.org>;tag=scscf\r\n" +
		"To: <sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org>\r\n" +
		"Call-ID: reg-1\r\n" +
		"CSeq: 10 REGISTER\r\n" +
		"Contact: <sip:311435300070581@192.168.105.116:37457;transport=udp>;expires=1700;audio;+g.3gpp.icsi-ref=\"urn%3Aurn-7%3A3gpp-service.ims.icsi.mcptt\";+g.3gpp.mcptt\r\n" +
		"Content-Length: 0\r\n\r\n"
	var resp []byte
	s.handleRaw(t.Context(), "10.90.250.52:5060", "udp", []byte(raw), func(b []byte) error {
		resp = append([]byte(nil), b...)
		return nil
	})
	...
	reg, err := st.GetRegistration(t.Context(), "sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org")
	if err != nil {
		t.Fatal(err)
	}
...
func TestThirdPartyRegisterEventWithoutFeatureTagCreatesRegistration(t *testing.T) {
	...
	raw := "REGISTER sip:mcptt-as.ims.mnc435.mcc311.3gppnetwork.org:5060 SIP/2.0\r\n" +
		...
		"CSeq: 10 REGISTER\r\n" +
		"Event: registration\r\n" +
		"Contact: <sip:scscf.ims.mnc435.mcc311.3gppnetwork.org>\r\n" +
		"Content-Length: 0\r\n\r\n"
	s.handleRaw(t.Context(), "10.90.250.52:5060", "udp", []byte(raw), func([]byte) error { return nil })
	reg, err := st.GetRegistration(t.Context(), "sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org")
	if err != nil {
		t.Fatal(err)
	}
...
func TestThirdPartyUnregisterExpiresRegistration(t *testing.T) {
	...
	s := NewServer(config.Default(), st)
	raw := "REGISTER sip:mcptt-as.ims.mnc435.mcc311.3gppnetwork.org:5060 SIP/2.0\r\n" +
		"Via: SIP/2.0/UDP 10.90.250.52:5060;branch=z9hG4bKunreg\r\n" +
		"From: <sip:scscf.ims.mnc435.mcc311.3gppnetwork.org>;tag=scscf\r\n" +
		"To: <sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org>\r\n" +
		"Call-ID: unreg-1\r\n" +
		"CSeq: 11 REGISTER\r\n" +
...
func TestInviteLifecycleTracksCallState(t *testing.T) {
	...
	cfg := config.Default()
	cfg.SIP.AdvertiseHost = "10.90.250.54"
	cfg.Media.AdvertiseHost = "10.90.250.54"
	s := NewServer(cfg, st)
	rawInvite := "INVITE sip:mcptt-as@ims.mnc435.mcc311.3gppnetwork.org SIP/2.0\r\n" +
		"Via: SIP/2.0/UDP 10.90.250.52:5060;branch=z9hG4bKinvite\r\n" +
		"From: <sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org>;tag=from1\r\n" +
		"To: <sip:mcptt-as@ims.mnc435.mcc311.3gppnetwork.org>\r\n" +
		"Call-ID: invite-1\r\n" +
		"CSeq: 1 INVITE\r\n" +
		"Contact: <sip:311435300070581@192.168.105.116:38897;transport=udp>\r\n" +
		"Content-Type: application/sdp\r\n\r\n" +
		"v=0\r\n" +
		"o=ue 0 0 IN IP4 192.168.105.116\r\n" +
		"s=MCPTT\r\n" +
	...
	s.handleRaw(t.Context(), "10.90.250.52:5060", "udp", []byte(rawInvite), func(b []byte) error {
	...
	if !strings.Contains(inviteResponses[2], "Contact: <sip:10.90.250.54:5060;transport=udp>") {
		t.Fatalf("200 OK missing advertised Contact:\n%s", inviteResponses[2])
	}
	if !strings.Contains(inviteResponses[2], "Record-Route: <sip:10.90.250.54:5060;transport=udp;lr>") {
	...
	if call == nil || call.State != "answered" || call.InitiatorURI != "sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org" || call.RemoteTarget == "" || call.SDPOffer == "" || call.SDPAnswer == "" {
		t.Fatalf("call after INVITE = %#v", call)
	}
	if call.AudioIP != "192.168.105.116" || call.AudioPort != 49170 || call.AudioProto != "RTP/AVP" || len(call.AudioPayloads) != 3 {
		t.Fatalf("audio media = %#v", call)
	}
	if call.FloorControlIP != "192.168.105.116" || call.FloorControlPort != 49172 || call.FloorControlProto != "UDP/MCPTT" || len(call.FloorControlAttrs) != 3 {
	...
	rawACK := "ACK sip:mcptt-as@ims.mnc435.mcc311.3gppnetwork.org SIP/2.0\r\n" +
		"Via: SIP/2.0/UDP 10.90.250.52:5060;branch=z9hG4bKack\r\n" +
		"From: <sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org>;tag=from1\r\n" +
		"To: <sip:mcptt-as@ims.mnc435.mcc311.3gppnetwork.org>;tag=" + localTag + "\r\n" +
		"Call-ID: invite-1\r\n" +
		"CSeq: 1 ACK\r\n" +
		"Content-Length: 0\r\n\r\n"
	s.handleRaw(t.Context(), "10.90.250.52:5060", "udp", []byte(rawACK), func([]byte) error { return nil })
	call, err = st.GetCall(t.Context(), "invite-1")
	if err != nil {
		t.Fatal(err)
	}
	...
	rawBYE := "BYE sip:mcptt-as@ims.mnc435.mcc311.3gppnetwork.org SIP/2.0\r\n" +
		"Via: SIP/2.0/UDP 10.90.250.52:5060;branch=z9hG4bKbye\r\n" +
		"From: <sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org>;tag=from1\r\n" +
		"To: <sip:mcptt-as@ims.mnc435.mcc311.3gppnetwork.org>;tag=" + localTag + "\r\n" +
		"Call-ID: invite-1\r\n" +
		"CSeq: 2 BYE\r\n" +
		"Content-Length: 0\r\n\r\n"
	var byeResp []byte
	s.handleRaw(t.Context(), "10.90.250.52:5060", "udp", []byte(rawBYE), func(b []byte) error {
...
func TestGroupInviteRequiresMembership(t *testing.T) {
	...
	s := NewServer(config.Default(), st)
	var rejected []string
	s.handleRaw(ctx, "10.90.250.52:5060", "udp", []byte(groupInvite("group-reject", "sip:u@example.test", nonMemberGroup.URI)), func(b []byte) error {
	...
	var accepted []string
	s.handleRaw(ctx, "10.90.250.52:5060", "udp", []byte(groupInvite("group-accept", "sip:u@example.test", memberGroup.URI)), func(b []byte) error {
...
func groupInvite(callID, fromURI, targetURI string) string {
	return "INVITE " + targetURI + " SIP/2.0\r\n" +
		"Via: SIP/2.0/UDP 10.90.250.52:5060;branch=z9hG4bK" + callID + "\r\n" +
		"From: <" + fromURI + ">;tag=from1\r\n" +
		"To: <" + targetURI + ">\r\n" +
		"Call-ID: " + callID + "\r\n" +
		"CSeq: 1 INVITE\r\n" +
		"Contact: <" + fromURI + "@192.168.105.116:38897;transport=udp>\r\n" +
		"Content-Type: application/sdp\r\n\r\n" +
		"v=0\r\n" +
		"o=ue 0 0 IN IP4 192.168.105.116\r\n" +
		"s=MCPTT\r\n" +
...
func TestUnknownBYEReturns481(t *testing.T) {
	...
	s := NewServer(config.Default(), st)
	rawBYE := "BYE sip:mcptt-as@ims.mnc435.mcc311.3gppnetwork.org SIP/2.0\r\n" +
		"Via: SIP/2.0/UDP 10.90.250.52:5060;branch=z9hG4bKunknownbye\r\n" +
		"From: <sip:311435300070581@ims.mnc435.mcc311.3gppnetwork.org>;tag=from1\r\n" +
		"To: <sip:mcptt-as@ims.mnc435.mcc311.3gppnetwork.org>;tag=to1\r\n" +
		"Call-ID: unknown-bye\r\n" +
		"CSeq: 2 BYE\r\n" +
		"Content-Length: 0\r\n\r\n"
	var resp []byte
...
func TestMediaEndpoint(t *testing.T) {
	if got := mediaEndpoint("192.168.105.116", 13400); got != "192.168.105.116:13400" {
		t.Fatalf("mediaEndpoint = %q", got)
	}
	if got := mediaEndpoint("", 13400); got != "" {
		t.Fatalf("empty host mediaEndpoint = %q", got)
	}
	if got := mediaEndpoint("192.168.105.116", 0); got != "" {
